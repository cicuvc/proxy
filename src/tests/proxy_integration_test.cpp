...
namespace proxy_integration_tests_details {
    ...
    interface_def(Drawable)
        fn_def(Draw, void(std::ostream&));
        fn_def(Area, double() noexcept);
    interface_end(Drawable);

    interface_def(Logger);
        fn_def(Log, void(const char*), void(const char*, const std::exception&));
    interface_end(Logger);
    ...
    public:
        ...
        double Area() const noexcept { return pi * radius_ * radius_; }
        ...
        if (!parsed.empty()) {
            if (parsed[0u] == "Rectangle") {
                if (parsed.size() == 3u) {
                    ...
                    std::pmr::polymorphic_allocator<Rectangle> alloc { &rectangle_memory_pool };
